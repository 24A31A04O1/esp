...
@app.route("/on/<device_name>")
def on(device_name):
    print("ON clicked for:", device_name)   # 
    supabase.table("devices").update(
        {"status": True}
    ).eq("device_name", device_name).execute()
    return redirect(url_for("index"))

@app.route("/off/<device_name>")
def off(device_name):
    print("OFF clicked for:", device_name)  # 🔴 DEBUG
    supabase.table("devices").update(
        {"status": False}
    ).eq("device_name", device_name).execute()
    return redirect(url_for("index"))
